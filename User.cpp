...
void Server::welcomeMsg(int &socket) {
	std::string response;
	int bytes_sent;
	std::string serverHostname(getServerHost());

	response = ":" + serverHostname + " 001 " + _clients[socket].get_nickname() + " :Welcome to the IRC Network, " + _clients[socket].get_nickname() + "!" + _clients[socket].get_username() + "@" + serverHostname + "\r\n";
