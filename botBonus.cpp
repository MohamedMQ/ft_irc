...
void sendFailedExit(int bytes_sent) {
	if (bytes_sent == -1)
	{
		delete fds;
		exit(-1);
					...
					if (!assembleCommand.empty()) {
						...
						while (posNL != std::string::npos) {
							...
							if (_cmd.find(":Password incorrect") != std::string::npos) {
								std::string quit;
								quit = "QUIT\n";
								bytes_sent = send(clientSocket, quit.c_str(), quit.size(), 0);
								sendFailedExit(bytes_sent);
								close (clientSocket);
								delete fds;
								return 1;
							...
							if (_cmd.find(":Password incorrect") == std::string::npos && firstStart == 1) {
								bytes_sent = send(clientSocket, "NICK TIGERSBOT\r\n", strlen("NICK TIGERSBOT\r\n"), 0);
								sendFailedExit(bytes_sent);
								bytes_sent = send(clientSocket, "USER TIGERSBOT 0 * TIGERSBOT\r\n", strlen("USER TIGERSBOT 0 * TIGERSBOT\r\n"), 0);
								sendFailedExit(bytes_sent);
								firstStart = 2;
							}
							if (!firstStart) {
								...
								firstStart = 1;
								bytes_sent = send(clientSocket, pas.c_str(), pas.size(), 0);
								sendFailedExit(bytes_sent);
							}
							if (firstStart == 2)
